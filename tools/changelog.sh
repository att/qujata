#x#!/usr/bin/env zsh
 ...
 function display-release {
   ...
   function fmt:hash {
     ...
     text)
       ...
       if supports_hyperlinks; then
         printf "\e]8;;%s\a%s\e]8;;\a" "https://github.com/att/qujata/commit/$hash" $text;
       else
         echo $text;
       fi ;;
     md) printf '[`%s`](https://github.com/att/qujata/commit/%s)' "$short_hash" "$hash" ;;
     esac
   }
   ...
   function fmt:subject {
     ...
     text)
       if supports_hyperlinks; then
         sed -E $'s|#([0-9]+)|\e]8;;https://github.com/att/qujata/issues/\\1\a\e[32m#\\1\e[0m\e]8;;\a|g;s|`([^`]+)`|`\e[2m\\1\e[0m`|g' <<< "$subject"
       else
         sed -E $'s|#([0-9]+)|\e[32m#\\1\e[0m|g;s|`([^`]+)`|`\e[2m\\1\e[0m`|g' <<< "$subject"
       fi ;;
     # In markdown mode, link to (#<issue>) issues
     md) sed -E 's|#([0-9]+)|[#\1](https://github.com/att/qujata/issues/\1)|g' <<< "$subject" ;;
     esac
   }
 ...
 function main {
   ...
   if [[ -z "$since" ]]; then
     ...
     # 1) try to find the first version tag before $until
   ...
   local version tag
   local hash refs subject bodydocs
