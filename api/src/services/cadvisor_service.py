...
def __build_k8s_metrics_url(service_name):
    pod = k8s_service.get_pod_by_prefix(service_name)
    cadvisor_pod = k8s_service.get_pod_by_prefix_and_host(CADVISOR_SERVICE_NAME, pod.status.host_ip)
    ...
    if cri == "docker":
        ...
        return K8S_CONTAINERD_CRI_METRCIS_URL.format(cadvisor_host, pod_uid, container_id)
    else:
